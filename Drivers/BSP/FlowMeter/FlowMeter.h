 ...
 * Flow Meter
 *
 * An STM32 flow meter library that provides calibrated liquid flow and volume measurement with flow sensors.
 *
...
class FlowMeter {
  public:
     ...
     */
    FlowMeter(uint32_t pin = GPIO_PIN_0, FlowSensorProperties prop = UncalibratedSensor, void (*callback)(void) = nullptr, uint8_t interruptMode = EXTI_MODE_RISING);
     ...
     */
    uint32_t getPin();                       //!< Returns the STM32 pin number that the flow sensor is connected to.
  ...
  protected:
    uint32_t _pin;                           //!< connection pin (has to be interrupt capable!)
